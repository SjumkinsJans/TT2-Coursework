    ...
    <div class="d-flex flex-wrap justify-content-center border border-dark w-100">
         ...
         <div class="box w-50 h-75 p-1" style="overflow:hidden;">
           <div class="box border border border-dark m-0 p-1 w-100">
                ...
                <input type="text" name="title" class="form-control" value="{{ old('title') ? old('title') : '' }}">
                <label for="description" class="form-label">Description</label>
                <textarea name="description" class="form-control w-100" style="resize:none;" rows="28">{{ old('description')}}
               </textarea>
               <select name="comic_genre_id" class="form-select">
               <option value="">Select Genre</option>
               @foreach($genres as $genre)
               <option value="{{$genre->id}}" @selected(old('comic_genre_id', '') == $genre->id) >{{$genre->comic_genre}}</option>
               @endforeach
               </select>
